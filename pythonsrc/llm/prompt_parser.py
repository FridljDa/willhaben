...
def decide_if_is_sublet(listing_content: dict[str, int]) -> LLmDecision:
  return decide_on_file_content(listing_content,
                                config.decide_if_is_sublet_prompt_filename)


def substitute_prompt_template(prompt_template_string: str,
    listing_content: dict[str, int]) -> str:
...
def call_llm_from_prompt_template(listing_content: dict[str, int],
    prompt_filename: str) -> (bool, str):
  ...
  print(
    f"DEBUG: llm answer decide_on_file_content for promptfile {prompt_filename}:\n{answer}")
  return success, answer


def decide_on_file_content(class_file_content: str,
    prompt_filename: str) -> LLmDecision:
  success, answer = call_llm_from_prompt_template(class_file_content,
                                                  prompt_filename)
...
def get_decision_from_llm_answer(answer: str) -> LLmDecision:
  is_positive_answer = any(
      [positive_string in answer for positive_string in positive_list])
  is_negative_answer = any(
      [negative_string in answer for negative_string in negative_list])
