...
class ListingsOverviewFetcher(Fetcher):
  ...
  def _extract_listings_summary(parsed_data: dict) -> list[dict]:
    ...
    """
    try:
      return parsed_data[KEY_PROPS][KEY_PAGE_PROPS][KEY_SEARCH_RESULT][
        KEY_LISTING_SUMMARY_LIST][KEY_LISTING_SUMMARY]
    except KeyError:
      logger.error("Failed to extract listing summaries due to missing keys.")
      return []

  def _process_single_listing(self,
      single_listing_before_conversion: dict) -> None:
    """
